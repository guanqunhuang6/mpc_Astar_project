function [feas, xOpt, uOpt, sol] = solve_cftoc3(P, Q, R, N, x0, u0, xL, xU, pointset)
...
% pointset
for i = 2:N+1
    points(:,i) = [0;pointset(i-1,1);pointset(i-1,2);0;0];
end
...
for k = 1:N
    constr = [constr, x(:,k+1) == dyn(x(:,k),u(:,k+1)), ...
        ...
        u(1,k+1) <= 8000, -8000 <= u(1,k+1), -8000 <= u(2,k+1), ...
        u(2,k+1) <= 8000 ];
    cost = cost + (x(:,k)-points(:,k))'*Q*(x(:,k)-points(:,k)) + ...
